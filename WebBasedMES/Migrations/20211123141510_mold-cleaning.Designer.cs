// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using WebBasedMES.Data;

namespace WebBasedMES.Migrations
{
    [DbContext(typeof(ApiDbContext))]
    [Migration("20211123141510_mold-cleaning")]
    partial class moldcleaning
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.8")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderKey")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<DateTime>("BirthDay")
                        .HasColumnType("datetime2");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("DepartmentId")
                        .HasColumnType("int");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<int?>("EmployTypeId")
                        .HasColumnType("int");

                    b.Property<string>("FullName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("IdNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("InDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsApproved")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsOut")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<DateTime>("OutDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<int?>("PositionId")
                        .HasColumnType("int");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TokenValidTime")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<int?>("UserRoleId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DepartmentId");

                    b.HasIndex("EmployTypeId");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.HasIndex("PositionId");

                    b.HasIndex("UserRoleId");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Defective", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatorId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("CreatorId");

                    b.ToTable("Defectives");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Facility", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Agent")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Brand")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<bool>("DailyInspection")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("MaxCurrent")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MaxTon")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Model")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PeriodicalInspection")
                        .HasColumnType("bit");

                    b.Property<int>("Price")
                        .HasColumnType("int");

                    b.Property<DateTime>("PurchaseDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Standard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Uid")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UploadFileId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("UploadFileId");

                    b.ToTable("Facilitys");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.InspectionItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Classify")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreateDateTime")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatorId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("InspectionCount")
                        .HasColumnType("int");

                    b.Property<string>("InspectionItems")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("InspectionType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("JudgeMethod")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("JudgeStandard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Type")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("CreatorId");

                    b.ToTable("InspectionItems");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.InspectionType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<string>("InspectionItem")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("InspectionMethod")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("InspectionStandard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Type")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.ToTable("InspectionTypes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Item", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("BuyPrice")
                        .HasColumnType("int");

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<bool>("ExportCheck")
                        .HasColumnType("bit");

                    b.Property<bool>("ImportCheck")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("OptimumStock")
                        .HasColumnType("int");

                    b.Property<int>("SellPrice")
                        .HasColumnType("int");

                    b.Property<string>("Standard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TaxType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Unit")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UploadFileId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("UploadFileId");

                    b.ToTable("Items");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Partner", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BankAccount")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BankName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusinessClass")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusinessNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusinessType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<string>("ContactEmail")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FaxNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("Group_Buy")
                        .HasColumnType("bit");

                    b.Property<bool>("Group_Etc")
                        .HasColumnType("bit");

                    b.Property<bool>("Group_Finance")
                        .HasColumnType("bit");

                    b.Property<bool>("Group_Sell")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PartnerType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("President")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PresidentNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TaxInfo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TelephoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.ToTable("Partners");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Process", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<bool>("FacilityUse")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("ProcessInspection")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.ToTable("Processes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessDefective", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("DefectiveId")
                        .HasColumnType("int");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<int?>("ProcessId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DefectiveId");

                    b.HasIndex("ProcessId");

                    b.ToTable("ProcessDefectives");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessDownTimeType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("DownTimeId")
                        .HasColumnType("int");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<int?>("ProcessId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DownTimeId");

                    b.HasIndex("ProcessId");

                    b.ToTable("ProcessDownTimeTypes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessFacility", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("FacilityId")
                        .HasColumnType("int");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<int?>("ProcessId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("FacilityId");

                    b.HasIndex("ProcessId");

                    b.ToTable("ProcessFacilities");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("OptimumStock")
                        .HasColumnType("int");

                    b.Property<string>("Standard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Unit")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UploadFileId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("UploadFileId");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProductItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("InputUnitId")
                        .HasColumnType("int");

                    b.Property<int?>("ItemId")
                        .HasColumnType("int");

                    b.Property<float>("Loss")
                        .HasColumnType("real");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.Property<int?>("ProcessId")
                        .HasColumnType("int");

                    b.Property<int>("ProcessNumber")
                        .HasColumnType("int");

                    b.Property<int>("ProcessOrder")
                        .HasColumnType("int");

                    b.Property<int?>("ProductId")
                        .HasColumnType("int");

                    b.Property<int>("RequiredAmount")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("InputUnitId");

                    b.HasIndex("ItemId");

                    b.HasIndex("ProcessId");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductItems");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProductProcess", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("IsFinal")
                        .HasColumnType("bit");

                    b.Property<bool>("IsOutSourcing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<int?>("PartnerId")
                        .HasColumnType("int");

                    b.Property<int?>("ProcessId")
                        .HasColumnType("int");

                    b.Property<int?>("ProductId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PartnerId");

                    b.HasIndex("ProcessId");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductProcesses");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfoCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("BaseInfoCodes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BusinessInfo", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Address")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusinessNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BusinessType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContactNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CorporationNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FaxNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("IndustryType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Introduce")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LogoUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("President")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("TaxRegistrationId")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("BusinessInfo");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.CommonCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatorId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("SortCodeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CreatorId");

                    b.HasIndex("SortCodeId");

                    b.ToTable("CommonCodes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Department", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Departments");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.EmployType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("EmployTypes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.JwtAuth.RefreshToken", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("AccessToken")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("AddedDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("ExpiryDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsRevorked")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsed")
                        .HasColumnType("bit");

                    b.Property<string>("JwtId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Token")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("RefreshTokens");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Menu", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("ApplicationUserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Icon")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAbleToAccess")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToDelete")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToRead")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToReadWrite")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Path")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Type")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationUserId");

                    b.ToTable("Menus");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.Draft", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Classify")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MoldDraftId")
                        .HasColumnType("int");

                    b.Property<DateTime>("RegisterDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("RegistererId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int?>("UploadFileId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("MoldDraftId");

                    b.HasIndex("RegistererId");

                    b.HasIndex("UploadFileId");

                    b.ToTable("Drafts");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.Mold", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("CleaningCycleId")
                        .HasColumnType("int");

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("CommonCodeId")
                        .HasColumnType("int");

                    b.Property<int>("CurrentCount")
                        .HasColumnType("int");

                    b.Property<bool>("DailyInspection")
                        .HasColumnType("bit");

                    b.Property<int>("DailyInspectionThreshold")
                        .HasColumnType("int");

                    b.Property<DateTime>("DiscardDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("DiscardReason")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DiscardRegistererId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("GuranteeCount")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Material")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("MoldCreateDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("OwenerId")
                        .HasColumnType("int");

                    b.Property<int>("Price")
                        .HasColumnType("int");

                    b.Property<DateTime>("RegisterDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("RegistererId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<bool>("RegularInspection")
                        .HasColumnType("bit");

                    b.Property<int>("RegularInspectionThreshold")
                        .HasColumnType("int");

                    b.Property<string>("Standard")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("StartCount")
                        .HasColumnType("int");

                    b.Property<string>("Status")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("UploadFileId")
                        .HasColumnType("int");

                    b.Property<int>("Weight")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CleaningCycleId");

                    b.HasIndex("CommonCodeId");

                    b.HasIndex("DiscardRegistererId");

                    b.HasIndex("OwenerId");

                    b.HasIndex("RegistererId");

                    b.HasIndex("UploadFileId");

                    b.ToTable("Molds");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldCleaning", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("CurrentCleaningDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Flag")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MoldId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("MoldId");

                    b.ToTable("MoldCleanings");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldCleaningElement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("CleaningClassifyId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CleaningDate")
                        .HasColumnType("datetime2");

                    b.Property<int?>("CleaningTypeId")
                        .HasColumnType("int");

                    b.Property<int>("GuranteeCount")
                        .HasColumnType("int");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MoldCleaningId")
                        .HasColumnType("int");

                    b.Property<int>("StartCount")
                        .HasColumnType("int");

                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("CleaningClassifyId");

                    b.HasIndex("CleaningTypeId");

                    b.HasIndex("MoldCleaningId");

                    b.HasIndex("UserId");

                    b.ToTable("MoldCleaningElements");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldDraft", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int?>("MoldId")
                        .HasColumnType("int");

                    b.Property<DateTime>("RegisterDate")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("UpdateDate")
                        .HasColumnType("datetime2");

                    b.HasKey("Id");

                    b.HasIndex("MoldId");

                    b.ToTable("MoldDrafts");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldGroup", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Flag")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAuto")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("MoldGroups");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldGroupElement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("FacilityId")
                        .HasColumnType("int");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MoldGroupId")
                        .HasColumnType("int");

                    b.Property<int?>("MoldId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("FacilityId");

                    b.HasIndex("MoldGroupId");

                    b.HasIndex("MoldId");

                    b.ToTable("MoldGroupElements");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Notice", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Content")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreateOn")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreatorId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Title")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CreatorId");

                    b.ToTable("Notices");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Position", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Positions");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.SortCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("SortCodes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.SubMenu", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<bool>("IsAbleToAccess")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToDelete")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToRead")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToReadWrite")
                        .HasColumnType("bit");

                    b.Property<bool>("IsFavorite")
                        .HasColumnType("bit");

                    b.Property<int>("MenuId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Path")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Type")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("MenuId");

                    b.ToTable("Submenus");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.UploadFile", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("FacilityId")
                        .HasColumnType("int");

                    b.Property<string>("FileName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FileStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FileUrl")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("MoldId")
                        .HasColumnType("int");

                    b.Property<int?>("NoticeId")
                        .HasColumnType("int");

                    b.Property<int?>("PartnerId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("FacilityId");

                    b.HasIndex("MoldId");

                    b.HasIndex("NoticeId");

                    b.HasIndex("PartnerId");

                    b.ToTable("UploadFiles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.UserRole", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Code")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsUsing")
                        .HasColumnType("bit");

                    b.Property<string>("Memo")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("UserRoles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Utilities.DefaultMenu", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Icon")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAbleToAccess")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToDelete")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToRead")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToReadWrite")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Path")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("DefaultMenus");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Utilities.DefaultSubMenu", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("DefaultMenuId")
                        .HasColumnType("int");

                    b.Property<bool>("IsAbleToAccess")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToDelete")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToRead")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAbleToReadWrite")
                        .HasColumnType("bit");

                    b.Property<bool>("IsFavorite")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Order")
                        .HasColumnType("int");

                    b.Property<string>("Path")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("DefaultMenuId");

                    b.ToTable("DefaultSubmenus");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.ApplicationUser", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Department", "Department")
                        .WithMany()
                        .HasForeignKey("DepartmentId");

                    b.HasOne("WebBasedMES.Data.Models.EmployType", "EmployType")
                        .WithMany()
                        .HasForeignKey("EmployTypeId");

                    b.HasOne("WebBasedMES.Data.Models.Position", "Position")
                        .WithMany()
                        .HasForeignKey("PositionId");

                    b.HasOne("WebBasedMES.Data.Models.UserRole", "UserRole")
                        .WithMany()
                        .HasForeignKey("UserRoleId");

                    b.Navigation("Department");

                    b.Navigation("EmployType");

                    b.Navigation("Position");

                    b.Navigation("UserRole");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Defective", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorId");

                    b.Navigation("CommonCode");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Facility", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.UploadFile", "UploadFile")
                        .WithMany()
                        .HasForeignKey("UploadFileId");

                    b.Navigation("CommonCode");

                    b.Navigation("UploadFile");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.InspectionItem", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorId");

                    b.Navigation("CommonCode");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.InspectionType", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.Navigation("CommonCode");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Item", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.UploadFile", "UploadFile")
                        .WithMany()
                        .HasForeignKey("UploadFileId");

                    b.Navigation("CommonCode");

                    b.Navigation("UploadFile");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Partner", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.Navigation("CommonCode");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Process", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.Navigation("CommonCode");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessDefective", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Defective", "Defective")
                        .WithMany()
                        .HasForeignKey("DefectiveId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Process", "Process")
                        .WithMany("ProcessDefectives")
                        .HasForeignKey("ProcessId");

                    b.Navigation("Defective");

                    b.Navigation("Process");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessDownTimeType", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "DownTime")
                        .WithMany()
                        .HasForeignKey("DownTimeId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Process", "Process")
                        .WithMany("ProcessDownTimeTypes")
                        .HasForeignKey("ProcessId");

                    b.Navigation("DownTime");

                    b.Navigation("Process");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProcessFacility", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Facility", "Facility")
                        .WithMany()
                        .HasForeignKey("FacilityId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Process", "Process")
                        .WithMany("ProcessFacilitys")
                        .HasForeignKey("ProcessId");

                    b.Navigation("Facility");

                    b.Navigation("Process");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Product", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.UploadFile", "UploadFile")
                        .WithMany()
                        .HasForeignKey("UploadFileId");

                    b.Navigation("CommonCode");

                    b.Navigation("UploadFile");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProductItem", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "InputUnit")
                        .WithMany()
                        .HasForeignKey("InputUnitId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Item", "Item")
                        .WithMany()
                        .HasForeignKey("ItemId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Process", "Process")
                        .WithMany()
                        .HasForeignKey("ProcessId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Product", "Product")
                        .WithMany("Items")
                        .HasForeignKey("ProductId");

                    b.Navigation("InputUnit");

                    b.Navigation("Item");

                    b.Navigation("Process");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.ProductProcess", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Partner", "Partner")
                        .WithMany()
                        .HasForeignKey("PartnerId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Process", "Process")
                        .WithMany()
                        .HasForeignKey("ProcessId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Product", "Product")
                        .WithMany("Processes")
                        .HasForeignKey("ProductId");

                    b.Navigation("Partner");

                    b.Navigation("Process");

                    b.Navigation("Product");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.CommonCode", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorId");

                    b.HasOne("WebBasedMES.Data.Models.SortCode", "SortCode")
                        .WithMany("CommonCodes")
                        .HasForeignKey("SortCodeId");

                    b.Navigation("Creator");

                    b.Navigation("SortCode");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.JwtAuth.RefreshToken", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "User")
                        .WithMany()
                        .HasForeignKey("UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Menu", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", null)
                        .WithMany("Menu")
                        .HasForeignKey("ApplicationUserId");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.Draft", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Mold.MoldDraft", "MoldDraft")
                        .WithMany("Drafts")
                        .HasForeignKey("MoldDraftId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Registerer")
                        .WithMany()
                        .HasForeignKey("RegistererId");

                    b.HasOne("WebBasedMES.Data.Models.UploadFile", "UploadFile")
                        .WithMany()
                        .HasForeignKey("UploadFileId");

                    b.Navigation("MoldDraft");

                    b.Navigation("Registerer");

                    b.Navigation("UploadFile");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.Mold", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CleaningCycle")
                        .WithMany()
                        .HasForeignKey("CleaningCycleId");

                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CommonCode")
                        .WithMany()
                        .HasForeignKey("CommonCodeId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "DiscardRegisterer")
                        .WithMany()
                        .HasForeignKey("DiscardRegistererId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Partner", "Owener")
                        .WithMany()
                        .HasForeignKey("OwenerId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Registerer")
                        .WithMany()
                        .HasForeignKey("RegistererId");

                    b.HasOne("WebBasedMES.Data.Models.UploadFile", "UploadFile")
                        .WithMany()
                        .HasForeignKey("UploadFileId");

                    b.Navigation("CleaningCycle");

                    b.Navigation("CommonCode");

                    b.Navigation("DiscardRegisterer");

                    b.Navigation("Owener");

                    b.Navigation("Registerer");

                    b.Navigation("UploadFile");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldCleaning", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Mold.Mold", "Mold")
                        .WithMany()
                        .HasForeignKey("MoldId");

                    b.Navigation("Mold");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldCleaningElement", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CleaningClassify")
                        .WithMany()
                        .HasForeignKey("CleaningClassifyId");

                    b.HasOne("WebBasedMES.Data.Models.CommonCode", "CleaningType")
                        .WithMany()
                        .HasForeignKey("CleaningTypeId");

                    b.HasOne("WebBasedMES.Data.Models.Mold.MoldCleaning", "MoldCleaning")
                        .WithMany("MoldCleaningList")
                        .HasForeignKey("MoldCleaningId");

                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "User")
                        .WithMany()
                        .HasForeignKey("UserId");

                    b.Navigation("CleaningClassify");

                    b.Navigation("CleaningType");

                    b.Navigation("MoldCleaning");

                    b.Navigation("User");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldDraft", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Mold.Mold", "Mold")
                        .WithMany()
                        .HasForeignKey("MoldId");

                    b.Navigation("Mold");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldGroupElement", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Facility", "Facility")
                        .WithMany()
                        .HasForeignKey("FacilityId");

                    b.HasOne("WebBasedMES.Data.Models.Mold.MoldGroup", "MoldGroup")
                        .WithMany("MoldGroupList")
                        .HasForeignKey("MoldGroupId");

                    b.HasOne("WebBasedMES.Data.Models.Mold.Mold", "Mold")
                        .WithMany()
                        .HasForeignKey("MoldId");

                    b.Navigation("Facility");

                    b.Navigation("Mold");

                    b.Navigation("MoldGroup");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Notice", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.ApplicationUser", "Creator")
                        .WithMany()
                        .HasForeignKey("CreatorId");

                    b.Navigation("Creator");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.SubMenu", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Menu", null)
                        .WithMany("Submenu")
                        .HasForeignKey("MenuId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.UploadFile", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Facility", null)
                        .WithMany("UploadFiles")
                        .HasForeignKey("FacilityId");

                    b.HasOne("WebBasedMES.Data.Models.Mold.Mold", null)
                        .WithMany("UploadFiles")
                        .HasForeignKey("MoldId");

                    b.HasOne("WebBasedMES.Data.Models.Notice", null)
                        .WithMany("UploadFiles")
                        .HasForeignKey("NoticeId");

                    b.HasOne("WebBasedMES.Data.Models.BaseInfo.Partner", null)
                        .WithMany("UploadFiles")
                        .HasForeignKey("PartnerId");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Utilities.DefaultSubMenu", b =>
                {
                    b.HasOne("WebBasedMES.Data.Models.Utilities.DefaultMenu", null)
                        .WithMany("Submenu")
                        .HasForeignKey("DefaultMenuId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.ApplicationUser", b =>
                {
                    b.Navigation("Menu");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Facility", b =>
                {
                    b.Navigation("UploadFiles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Partner", b =>
                {
                    b.Navigation("UploadFiles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Process", b =>
                {
                    b.Navigation("ProcessDefectives");

                    b.Navigation("ProcessDownTimeTypes");

                    b.Navigation("ProcessFacilitys");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.BaseInfo.Product", b =>
                {
                    b.Navigation("Items");

                    b.Navigation("Processes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Menu", b =>
                {
                    b.Navigation("Submenu");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.Mold", b =>
                {
                    b.Navigation("UploadFiles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldCleaning", b =>
                {
                    b.Navigation("MoldCleaningList");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldDraft", b =>
                {
                    b.Navigation("Drafts");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Mold.MoldGroup", b =>
                {
                    b.Navigation("MoldGroupList");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Notice", b =>
                {
                    b.Navigation("UploadFiles");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.SortCode", b =>
                {
                    b.Navigation("CommonCodes");
                });

            modelBuilder.Entity("WebBasedMES.Data.Models.Utilities.DefaultMenu", b =>
                {
                    b.Navigation("Submenu");
                });
#pragma warning restore 612, 618
        }
    }
}
