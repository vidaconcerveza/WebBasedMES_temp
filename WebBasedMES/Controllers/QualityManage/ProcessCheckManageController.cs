...
{
    public class ProcessCheckManageController : ControllerBase
    {
        private readonly ILogger<ProcessCheckManageController> _logger;
        private readonly IProcessCheckMngRepository _repo;
        private readonly ApiDbContext _db;
        private readonly UserManager<ApplicationUser> _userManager;

        public ProcessCheckManageController(ILogger<ProcessCheckManageController> logger, IProcessCheckMngRepository repo, UserManager<ApplicationUser> userManager, ApiDbContext db)
        {
            _logger = logger;
            _repo = repo;
            _db = db;
        ...
        // 1) 공정검사현황 메인 화면
        [HttpPost("/api/quality/processCheck/mst/list")]
        public async Task<IActionResult> processCheckMstList([FromBody] ProcessCheckReq001 processCheckReq001, [FromHeader(Name = "Uuid")] string Uuid)
        {
            
            var result = await _repo.processCheckMstList(processCheckReq001);
            if (result.IsSuccess)
            {
